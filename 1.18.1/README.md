...
`+0 added` · `-0 removed` · `~57 modified`

Fog is reworked from a spherical to a **cylindrical** distance model, so fog density no longer changes as you look up and down.

- `include/fog.glsl` gains `cylindrical_distance()`, which returns `max(horizontal distance, vertical distance)` instead of the straight-line distance
- a new `IViewRotMat` (inverse view rotation) uniform is added to 21 shaders and their JSONs, so the distance can be measured in world space rather than view space
- every `vertexDistance = length((ModelViewMat * vec4(Position, 1.0)).xyz);` is replaced by a `cylindrical_distance(...)` call

<details>
<summary><b>Modified (57)</b></summary>
...
◀ [1.18](../1.18/README.md) · [All versions](../README.md) · [1.18.2](../1.18.2/README.md) ▶
